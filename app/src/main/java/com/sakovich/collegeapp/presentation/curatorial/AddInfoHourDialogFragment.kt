...
class AddInfoHourDialogFragment : DialogFragment() {
    ...
    companion object {
        fun newInstance(groupName: String, teacherName: String, teacherId: String): AddInfoHourDialogFragment {
            return AddInfoHourDialogFragment().apply {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val dialog = MaterialAlertDialogBuilder(requireContext())
            .setTitle("Добавить информационный час")
            .setView(view)
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                val idsToNotify = selectedIds.toList()
                val schedule = ScheduleItem(
                    ...
                    room = room,
                    type = ScheduleType.INFO_HOUR,
                    group = groupName,
                    createdBy = teacherId,
                    createdByRole = "teacher",
                    assignedStudentIds = idsToNotify
                )
                lifecycleScope.launch {
                    try {
                        val scheduleId = withContext(Dispatchers.IO) {
                            scheduleRepository.addSchedule(schedule)
                        }
                        if (idsToNotify.isNotEmpty()) {
                            withContext(Dispatchers.IO) {
                                val message = "Тема: «$topic». Дата: $selectedDate, $time. Подготовьте выступление к информационному часу."
                                notificationRepository.createGroupNotification(
                                    studentIds = idsToNotify,
                                    title = "Подготовьте тему информационного часа",
                                    message = message,
                        ...
                        Toast.makeText(
                            ctx,
                            if (idsToNotify.isNotEmpty())
                                "Информационный час добавлен. Напоминания отправлены ${idsToNotify.size} учащимся."
                            else
                                "Информационный час добавлен в расписание.",
                            Toast.LENGTH_SHORT
                        ).show()
